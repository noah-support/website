...
/**
 * Shader-based agent aura. Visual language ported from the Unicorn Studio /
 * LiveKit agents-ui aura (Polyform Non-Resale 1.0.0, © 2026 UNCRN LLC),
 * with a duotone ramp and raw WebGL1 renderer from `temp/aura.html`.
 * Volume is driven by ElevenLabs `getInputVolume` / `getOutputVolume`.
 */
...
function targetsFor(state: ShaderState, vol: number): ParamSet {
  ...
  if (state === "speaking") {
    s.speed += vol * 0.12;
    s.amplitude += vol * 0.48;
    s.frequency += vol * 0.06;
    s.scale += vol * 0.09;
    s.brightness += vol * 0.55;
      ...
      const volTarget =
        shaderState === "speaking"
          ...
          : shaderState === "listening"
            ? input
            : 0;
      vol = ease(vol, volTarget, dt * (volTarget > vol ? 5 : 3));

      const tgt = targetsFor(shaderState, vol);
      for (const key of AUTO_KEYS) {
        params[key] = ease(params[key], tgt[key], dt * 1.6);
      }
